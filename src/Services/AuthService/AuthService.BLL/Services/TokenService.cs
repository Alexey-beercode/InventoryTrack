...
namespace AuthService.BLL.Services
{
    public class TokenService : ITokenService
    {
        ...
        private List<Claim> CreateClaims(User user, IEnumerable<Role> roles)
        {
            ...
            {
                var translatedRole = TranslateRoleToEnglish(role.Name);
                return new Claim(ClaimTypes.Role, translatedRole);
            }));

            return claims;
        }

        private string TranslateRoleToEnglish(string roleName)
        {
            return roleName switch
            {
                "Начальник подразделения" => "Department Head",
                "Пользователь" => "Resident",
                "Начальник склада" => "Warehouse Manager",
