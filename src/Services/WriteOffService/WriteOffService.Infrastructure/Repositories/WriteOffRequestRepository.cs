...
public class WriteOffRequestRepository : BaseRepository<WriteOffRequest> , IWriteOffRequestRepository
{
    ...
    {
        return await _dbSet.Where(request => request.ApprovedByUserId == userId && !request.IsDeleted)
    ...
    {
        return await _dbSet.Where(request => request.WarehouseId == warehouseId && !request.IsDeleted)
            .ToListAsync(cancellationToken);
    }
    ...
    {
        return await _dbSet.Where(request => request.Status == status && !request.IsDeleted)
            .ToListAsync(cancellationToken);
    }
