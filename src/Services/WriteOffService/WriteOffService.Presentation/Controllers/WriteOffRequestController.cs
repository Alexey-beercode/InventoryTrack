...
public class WriteOffRequestController : ControllerBase
{
    ...
    // Добавить эти методы в WriteOffRequestController
// Добавить этот метод в WriteOffRequestController
    ...
    [ProducesResponseType(StatusCodes.Status403Forbidden)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public async Task<IActionResult> CreateBatchWriteOff(
        [FromBody] CreateBatchWriteOffRequestDto createDto,  // FromForm для поддержки файлов
        CancellationToken cancellationToken)
    {
        try
        {
            _logger.LogInformation("Creating batch write-off request for batch: {BatchNumber}", createDto.BatchNumber);
            await _writeOffRequestService.CreateBatchWriteOffRequestAsync(createDto, cancellationToken);
        
            return Created("", new { 
                Message = $"Запросы на списание партии {createDto.BatchNumber} созданы",
                BatchNumber = createDto.BatchNumber 
            });
        }
        catch (EntityNotFoundException ex)
        {
            _logger.LogWarning("Batch not found: {BatchNumber} - {Error}", createDto.BatchNumber, ex.Message);
            return NotFound(new { Message = ex.Message, BatchNumber = createDto.BatchNumber });
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error creating batch write-off for batch: {BatchNumber}", createDto.BatchNumber);
