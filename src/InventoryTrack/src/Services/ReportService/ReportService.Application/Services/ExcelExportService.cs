    ...
    public class ExcelExportService : IExcelExportService
    {
        ...
        public void GenerateStockStateSheet(IXLWorksheet worksheet, BsonDocument data)
        {
            ...
            foreach (var item in items)
            {
                ...
                worksheet.Cell(row, 3).Value = bsonItem["Quantity"].ToInt64(); // Количество
                worksheet.Cell(row, 4).Value = bsonItem["EstimatedValue"].ToDecimal(); // Оценочная стоимость (Исправлено)
